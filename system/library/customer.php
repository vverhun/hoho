...
final class Customer {
	...
	public function __construct($registry) {
		...
		if (isset($this->session->data['customer_id'])) {
			...
			if ($customer_query->num_rows) {
				...
				$this->ansprechpartner = $customer_query->row['ansprechpartner'];
	...
	public function login2($email, $password) {
		if (!$this->config->get('config_customer_approval')) {
			$customer_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE LOWER(username) = '" . $this->db->escape(strtolower($email)) . "' AND password = '" . $this->db->escape(md5($password)) . "' AND status = '1'");
		} else {
			$customer_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE LOWER(username) = '" . $this->db->escape(strtolower($email)) . "' AND password = '" . $this->db->escape(md5($password)) . "' AND status = '1' AND approved = '1'");
		}
