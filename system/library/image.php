...
final class Image {
    ...
    public function resize($width = 0, $height = 0) {
		...
		$maxside = $this->info['width'];
		if ( $this->info['height'] > $this->info['width']) $maxside = $this->info['height'];
		
		$proportion  = 1;
		
		if ( $this->info['height'] > $this->info['width']) {
			$proportion = (int)$this->info['height']/(int)$this->info['width'];
 
			$new_height = (int)($height);
			$new_width = (int)($height/$proportion);
			
		}
		else {
			$proportion = (int)$this->info['width']/(int)$this->info['height'];
		    $new_width = (int)($width);			
