...
class ModelImportProduct extends Model {
	...
	public function getProductByImage($image) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.image = '".$image."'");

		return $query->rows;
	}
	
	/*public function getProductByImage($image) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product p WHERE p.image = '".$image."'");
		return $query->rows;
	}*/
