...
class ModelAccountOrder extends Model {
	...
	public function getOrders($start = 0, $limit = 20) {
		...
		$query = $this->db->query("SELECT o.order_id, o.firstname, o.lastname,  o.date_added, o.total, o.currency, o.order_status_id, o.value FROM `" . DB_PREFIX . "order` o  WHERE customer_id = '" . (int)$this->customer->getId() . "' AND o.order_status_id > '0'   ORDER BY o.order_id DESC ");
	...
	public function getOrders2( ) {
		...
		$query = $this->db->query("SELECT o.order_id, o.firstname, o.lastname,  o.date_added, o.total, o.currency,o.subtotal,o.tax, o.order_status_id, o.value FROM `" . DB_PREFIX . "order` o  WHERE customer_id = '" . (int)$this->customer->getId() . "' AND  o.order_status_id IN ('5','7')   ORDER BY o.order_id DESC ");
	...
	public function updateStatusData($orderId, $status, $data)
	{
		$query = 'UPDATE '.DB_PREFIX.'order SET order_status_id='.(int)$status.', date_modified = NOW(), ppData="'.$data.'" WHERE order_id='.(int)$orderId;
		$this->db->query($query);
		...
		if ( intval($status) == 5 ){
			...
			$query = $this->db->query("SELECT  invoice as invoice FROM `" . DB_PREFIX . "order` WHERE order_id=".(int)$orderId);
		    $invoice_check  =  $query->row['invoice'];
			
			if ( intval($invoice_check) == 0 || intval($invoice_check) == 1){
			    $this->db->query("UPDATE richterorder SET  invoice =  " . $invoice . "   WHERE order_id=".(int)$orderId);
			}
