...
class ModelAccountCustomer extends Model {
    ...
    public function addCustomer($data) {
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "customer SET  lang = '" . $this->db->escape($data['communic']) . "',  downloadedfile = '', username = '" . $this->db->escape($data['username']) . "', mobile = '" . $this->db->escape($data['mobile']) . "', bill_address = '" . $this->db->escape($data['bill_address']) . "', privatperson = '" . $this->db->escape($data['privatperson']) . "', museum = '" . $this->db->escape($data['museum']) . "', firma = '" . $this->db->escape($data['firma']) . "', taxid_number = '" . $this->db->escape($data['taxid_number']) . "', position = '" . $this->db->escape($data['position']) . "', ansprechpartner = '" . $this->db->escape($data['ansprechpartner']) . "', ansprechpartner2 = '" . $this->db->escape($data['ansprechpartner2']) . "', store_id = '" . (int) $this->config->get('config_store_id') . "', firstname = '" . $this->db->escape($data['company']) . "', lastname = '" . $this->db->escape($data['company2']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', password = '" . $this->db->escape(md5($data['password'])) . "', newsletter = '0', customer_group_id = '" . (int) $this->config->get('config_customer_group_id') . "', status = '1', date_added = NOW()");
    ...
    public function editCustomer($data) {
        ...
        $this->db->query("UPDATE " . DB_PREFIX . "customer SET lang = '" . $this->db->escape($data['communic']) . "', privatperson = '" . $this->db->escape($data['privatperson']) . "', museum = '" . $this->db->escape($data['museum']) . "', firma = '" . $this->db->escape($data['firma']) . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "',  taxid_number = '" . $this->db->escape($data['taxid_number']) . "', ansprechpartner = '" . $this->db->escape($data['ansprechpartner']) . "', ansprechpartner2 = '" . $this->db->escape($data['ansprechpartner2']) . "', position = '" . $this->db->escape($data['position']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', mobile = '" . $this->db->escape($data['mobile']) . "' WHERE customer_id = '" . (int) $this->customer->getId() . "'");
    ...
    public function getCustomerType($customer_id) {
        ...
        if (( $firma == 'on' || $museum == 'on' ) && $country == 'Germany')
            return 1;
        if ($firma != 'on' && $museum != 'on' && $country == 'Germany')
            return 2;
        if ($this->isEUState($country) && trim($taxid) != '' && ( $firma == 'on' || $museum == 'on' ))
            return 3;
        if ($this->isEUState($country) && trim($taxid) == '' && ( $firma == 'on' || $museum == 'on' ))
            return 4;
        if (!$this->isEUState($country) && ( $firma == 'on' || $museum == 'on'))
            return 5;
        if (!$this->isEUState($country) && ( $firma != 'on' && $museum != 'on'))
            return 6;
        if ($this->isEUState($country) && $firma != 'on' && $museum != 'on')
