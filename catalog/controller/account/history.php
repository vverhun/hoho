...
class ControllerAccountHistory extends Controller {
	...
	private function getOrdersList1(){
		...
		foreach ($results as $result) {
			...
			if ( $status == '1'){ $this->data['orders_total1']++; $status = "<span style='color:red'>".$this->data['my_offen']."</span>";}
			if ( $status == '3'){ $this->data['orders_total2']++;  $status = "<span style='color:blue'>".$this->data['my_angebot']."</span>";}
	...
	private function getOrdersList2(){
		...
		foreach ($results as $result) {
			...
			if ( $status == '5') $status = "<span style='color:green'>".$this->data['my_bezahlt']."</span>";
			if ( $status == '7') $status = "<span style='color:#404042;'>".$this->data['my_abgelehnt']."</span>";
			$this->data['orders_total3'] ++;
			$this->data['orders2'][] = array(
          			'order_id'   => $result['order_id'],
					'order_status_id' => $result['order_status_id'],
          			'name'       => $result['firstname'] . ' ' . $result['lastname'],
