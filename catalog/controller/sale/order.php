...
class ControllerSaleOrder extends Controller {
    ...
    public function offen() {
        ...
        $this->data['sort_order'] = HTTPS_SERVER . 'index.php?route=sale/order/anfragen&sort=o.order_id' . $url;
        $this->data['ansprechpartner_order'] = HTTPS_SERVER . 'index.php?route=sale/order/anfragen&sort=c.ansprechpartner' . $url;
        $this->data['totalproducts_order'] = HTTPS_SERVER . 'index.php?route=sale/order/anfragen&sort=totalproducts' . $url;
        $this->data['date_order'] = HTTPS_SERVER . 'index.php?route=sale/order/anfragen&sort=o.date_added' . $url;
        $this->data['edit_order'] = HTTPS_SERVER . 'index.php?route=sale/order/anfragen&sort=date_modified' . $url;
        $this->data['status_order'] = HTTPS_SERVER . 'index.php?route=sale/order/anfragen&sort=o.order_status_id' . $url;
        $this->data['firstname_order'] = HTTPS_SERVER . 'index.php?route=sale/order/anfragen&sort=firstname' . $url;
        ...
        $this->getList(1, $sort, $order);
    }

    public function bearbeitete() {
        $this->load->language('sale/order');
        ...
        $this->data['sort_order'] = HTTPS_SERVER . 'index.php?route=sale/order/bearbeitete&sort=o.order_id' . $url;
        $this->data['ansprechpartner_order'] = HTTPS_SERVER . 'index.php?route=sale/order/bearbeitete&sort=c.ansprechpartner' . $url;
        $this->data['totalproducts_order'] = HTTPS_SERVER . 'index.php?route=sale/order/bearbeitete&sort=totalproducts' . $url;
        $this->data['date_order'] = HTTPS_SERVER . 'index.php?route=sale/order/bearbeitete&sort=o.date_added' . $url;
        $this->data['edit_order'] = HTTPS_SERVER . 'index.php?route=sale/order/bearbeitete&sort=date_modified' . $url;
        $this->data['status_order'] = HTTPS_SERVER . 'index.php?route=sale/order/bearbeitete&sort=o.order_status_id' . $url;
        $this->data['firstname_order'] = HTTPS_SERVER . 'index.php?route=sale/order/bearbeitete&sort=firstname' . $url;
    ...
    public function projektarchiv() {
        ...
        $quater = null;
        if (isset($this->request->get['quater'])) {
            $quater = $this->request->get['quater'];
        }

        $this->data['year'] = $year;
        $this->data['quater'] = $quater;
        $this->data['type'] = $type;

        $url = '&year=' . $year . '&type=' . $type . '&quater=' . $quater;
        ...
        $this->data['sort_order'] = HTTPS_SERVER . 'index.php?route=sale/order/projektarchiv&sort=o.order_id' . $url;
        $this->data['ansprechpartner_order'] = HTTPS_SERVER . 'index.php?route=sale/order/projektarchiv&sort=c.ansprechpartner' . $url;
        $this->data['totalproducts_order'] = HTTPS_SERVER . 'index.php?route=sale/order/projektarchiv&sort=totalproducts' . $url;
        $this->data['date_order'] = HTTPS_SERVER . 'index.php?route=sale/order/projektarchiv&sort=o.date_added' . $url;
        $this->data['modified_order'] = HTTPS_SERVER . 'index.php?route=sale/order/projektarchiv&sort=o.invoice' . $url;


        $this->data['edit_order'] = HTTPS_SERVER . 'index.php?route=sale/order/projektarchiv&sort=date_modified' . $url;
        $this->data['status_order'] = HTTPS_SERVER . 'index.php?route=sale/order/projektarchiv&sort=o.order_status_id' . $url;
        $this->data['firstname_order'] = HTTPS_SERVER . 'index.php?route=sale/order/projektarchiv&sort=firstname' . $url;
        ...
        $this->getList2($this->request->get['type'], $sort, $order, $year, $quater);
    }
    ...
    public function delete() {
        ...
        if (isset($this->request->get['order_id'])) {
            ...
            $this->redirect(HTTPS_SERVER . 'index.php?route=sale/order/' . $back . '&token=' . $this->session->data['token'] . $url);
        }
    }

    private function getList($filter_order_status_id, $sort, $order) {
        ...
        /*
          if ( $status == 1 ){
          $filter_order_status_id = "1, 3";
          }

          if ( $status == 2 ){
          $filter_order_status_id = "5, 7";
        ...
        $offen_total = $this->model_sale_order->getTotalOrders($data);
        $this->data['offen'] = $offen_total;
        ...
        $angebot_total = $this->model_sale_order->getTotalOrders($data);
        $this->data['angebot'] = $angebot_total;
        ...
        foreach ($results as $result) {
            ...
            $totalproducts = $this->model_sale_order->getOrderProducts($result['order_id']);
            $status_name = '<span style="color:red;">offen</span>';

            if ($result['status'] == '3')
                $status_name = '<span style="color:blue;">Angebot</span>';

            $products_order = array();

            foreach ($totalproducts as $order_product) {
                ...
                $products_order[] = array(
                    ...
                    'realprice' => ($order_product['price'] * 100) / (100 + $order_product['pricediscount']),
                    'art' => $art[$order_product['art']],
                    'art_id' => $order_product['art'],
                    'auflage' => $order_product['auflage'],
                    'pricediscount' => (is_null($order_product['pricediscount'])) ? 0 : $order_product['pricediscount'],
                    'taxdiscount' => (is_null($order_product['taxdiscount'])) ? 0 : $order_product['taxdiscount'],
                    ...
                    'thumb' => $this->model_tool_image->resize($image, 90, 90)
                );
            }
        ...
        $this->template = 'default/template/sale/order_list' . $filter_order_status_id . '.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    private function getList2($status, $sort, $order, $year, $quater) {
        ...
        foreach ($results as $result) {
            ...
            if ($result['status'] == '5') {
                $status_name = '<span style="color:green;">abgerechnet</span>';
            }
            if ($result['status'] == '7') {
                $status_name = '<span style="color:#404042;">abgelehnt</span>';
            }
            ...
            $this->data['orders'][] = array(
                ...
                'action' => $action,
                'numberpercent' => $numberpercent,
                'pdfid' => 'C ' . $this->model_sale_order->getPaidOrderNumber($result['order_id'], $result['date_modified']) . '/' . date('Y', strtotime($result['date_modified']))
            );
        }
        ...
        $this->template = 'default/template/sale/order_list5.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    public function getForm() {
        ...
        if ($order_info) {
            ...
            $this->template = 'default/template/sale/order_form.tpl';
            $this->children = array(
                'common/header',
                'common/footer'
            );
    ...
    public function invoice() {
        ...
        $this->template = 'default/template/sale/order_invoice.tpl';
    ...
    public function angebot() {
        ...
        $totalproducts = $this->model_sale_order->getOrderProducts($order_id);
        $totalsum = 0;
        foreach ($totalproducts as $order_product) {
            $product_id = $order_product['product_id'];
            $price = intval($this->request->get['first' . $product_id]);
            $tax = intval($this->request->get['second' . $product_id]);
            $realprice = intval($this->request->get['realprice1' . $product_id]);

            $originaltax = $order_product['tax'];
            $originalprice = $order_product['price'];
            if (!empty($realprice)) {
                $originalprice = $realprice; //angebot bearbeiten
                $originaltax = 50;
            }
            $originalprice = $originalprice + (($originalprice * $price) / 100);
            $originaltax = $originaltax + (($originaltax * $tax) / 100);

            $totalproduct = $originalprice + $originaltax;

            //update totalproduct
            if (in_array($product_id, $del)) {
                $this->model_sale_order->updateProductPrices(0, 0, 0, $product_id, $order_id);
                $this->model_sale_order->updateProductStatus(0, $product_id, $order_id);
            } else {
                $this->model_sale_order->updateProductPrices($originalprice, $originaltax, $totalproduct, $product_id, $order_id, $price, $tax);
                $totalsum = $totalsum + $totalproduct;
            }
        }

        $this->load->model('account/customer');
        $order = $this->model_sale_order->getOrder($order_id);
        $customer_type = $this->model_account_customer->getCustomerType($order['customer_id']);
        ...
        if ($customer_type == 1 || $customer_type == 2 || $customer_type == 4 or $customer_type == 7) {
            $tax = $subtotal * 0.07;
            $totalsum = $subtotal + $tax;
        } else {
            $subtotal = $totalsum;
            $tax = 0;
        }

        //update totalsum and order status
        $this->model_sale_order->updateTotalPrice($totalsum, $subtotal, $tax, $order_id);
        $this->model_sale_order->updateOrderStatus('3', $order_id);
        ...
        /* //now
          $country = $this->model_account_customer->getCustomerCountry($order['customer_id']);
          ...
          if ($lang == 'de-DE') {
          $message = sprintf($this->language->get('text_email'), date($this->language->get('date_format_short'), strtotime($order['date_added'])));
          ...
          $message = sprintf($this->language->get('text_email_en'), date($this->language->get('date_format_short'), strtotime($order['date_added'])));
          $message .= $this->language->get('text_footer_email_en');
          $mail->setTo($order['email']);
          $mail->setFrom($this->config->get('config_email'));
          $mail->setSender($this->config->get('config_name'));
          $mail->setSubject($this->language->get('text_subject_en'));
          $mail->setHtml($message, ENT_QUOTES, 'UTF-8');
          @$mail->send();
          }

         */
        $this->redirect(HTTPS_SERVER . 'index.php?route=sale/order/anfragen');
    }
    ...
    public function cancel() {
        ...
        $this->model_sale_order->updateOrderStatus('7', $order_id);
        ...
        if ($country == 'Germany' || $country == 'Austria' || $country == 'Switzerland') {
            $mail->setTo($order['email']);
            $message = sprintf($this->language->get('text_email2'), date($this->language->get('date_format_short'), strtotime($order['date_added'])));
            $message .= $this->language->get('text_footer_email');

            $mail->setFrom($this->config->get('config_email'));
            $mail->setSender($this->config->get('config_name'));
            $mail->setSubject($this->language->get('text_subject2'));
            $mail->setHtml($message, ENT_QUOTES, 'UTF-8');
            //now @$mail->send();
        } else {
            $mail->setTo($order['email']);
            $message = sprintf($this->language->get('text_email2_en'), date($this->language->get('date_format_short'), strtotime($order['date_added'])));
            $message .= $this->language->get('text_footer_email_en');
    ...
    public function cancelclient() {
        ...
        $this->redirect(HTTPS_SERVER . 'index.php?route=account/history');
    }
    ...
    public function printorder() {
        ...
        if ($order_info) {
            ...
            $bill_info = $this->model_account_customer->getBill($order_info['customer_id']);
            $address_info = $this->model_sale_customer->getAddressesByCustomerId($order_info['customer_id']);
            ...
            foreach ($products as $product) {
                ...
                $sku = ( $product_info['sku'] == -1 ) ? '0' : $product_info['sku'];
                $sku_sort = $product_info['sku_sort'];
                if (intval($product['status_id']) != 0) {
                    $product_data[] = array(
                        'sku_sort' => $sku_sort,
                        'titelseite' => $product['titelseite'],
                        'name' => $product['name'],
                        'innenseite' => $product['innenseite'],
                        'tax' => $product['tax'],
                        'thumb' => $product['thumb'],
                        'product_id' => $product['product_id'],
                        'model' => $product['model'],
                        'titel' => $product['titel'],
                        'jahr' => $product_info['location'],
                        'auflage' => $product['auflage'],
                        'grosse' => $product['grosse'],
                        'art' => $product['art'],
                        'sku' => $sku,
            ...
            usort($product_data, 'custom_sort');


            $total_data = $this->model_sale_order->getOrderTotals($order_id);

            $date = $order_info['date_added'];
            $date = date('d.m.Y', strtotime($order_info['date_modified']));
            $year = date('Y', strtotime($order_info['date_modified']));

            $date_modified = $order_info['date_modified'];
            $date_modified = date('d.m.Y', strtotime($order_info['date_modified']));

            $this->data['orders'][] = array(
                'order_id' => $order_id,
                'date_added' => date($this->language->get('date_format_short'), strtotime($order_info['date_modified'])),
                'store_name' => $order_info['store_name'],
                'store_url' => rtrim($order_info['store_url'], '/'),
                'address' => nl2br($this->config->get('config_address')),
                'telephone' => $this->config->get('config_telephone'),
                ...
                'order_id' => $this->model_sale_order->getPaidOrderNumber($order_info['order_id'], $order_info['date_modified']),
                'year' => $year,
                'product' => $product_data,
                'firstname' => $customer_info['firstname'],
                'ansprechpartner' => $customer_info['ansprechpartner'],
                'taxid' => $customer_info['taxid_number'],
                'position' => $customer_info['position'],
                'lastname' => $customer_info['lastname'],
                'bill_address_1' => $address_info[0]['address_1'],
                'bill_city' => $address_info[0]['city'],
                'bill_land' => '',
                'bill_postcode' => $address_info[0]['postcode'],
                'country' => $address_info[0]['country_id']
            );
        }


        $customer_type = $this->model_account_customer->getCustomerType($order_info['customer_id']);
        ...
        if ($customer_type == '3' && $order_info['tax'] > 0)
            $customer_type = 4;

        $this->template = '/default/template/sale/order_invoice' . $customer_type . '.tpl';
    ...
    public function saveorder($order_id) {
        ...
        $template = '/default/template/sale/order_invoice' . $customer_type . '.tpl';

        //$this->response->setOutput($this->render(TRUE), $this->config->get('config_compression'));
        $this->renderPDFIntoFile($template, $order_id);
        return '/pdf/invoice' . $order_id . '.pdf';
    }
