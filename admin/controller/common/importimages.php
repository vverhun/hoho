...
class ControllerCommonImportimages extends Controller {
	...
	public function upload() {
							...
							if(is_file($img)) {
								...
								if (!in_array((int)$category_id, $catArr)){
									...
									if(isset($xmlObj['x:xmpmeta']['rdf:RDF']['rdf:Description']['dc:title']['rdf:Alt']['rdf:li'])) {
										...
										if(isset($names[1])) {
											...
											$errors[$i]['default_name'] = "Default Product Name German";
										}
									}
									else {
								...
								if(isset($xmlObj['x:xmpmeta']['rdf:RDF']['rdf:Description_attr']['Iptc4xmpCore:IntellectualGenre'])) {
									...
									if(isset($meta_description[1])) {
										...
										$errors[$i]['default_meta_description'] = "Default meta_description German";
									}
								}
								else {
								...
								$product_model = $this->model_catalog_product->getProductByImage($product['image']);
								...
								if(count($product_model) > 0) {
									...
									$errors[$i]['product_id'] = $this->model_catalog_product->addProduct($data);
								}
		...
		$this->template = 'common/import.tpl';
		$this->children = array(
			'common/header',	
			'common/footer'	
			);
